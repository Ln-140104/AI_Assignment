...
def graphemes(text): return len(regex.findall(r'\X',text))
def environment():
    lines=[f'python={sys.version}']
    for name in ['tiktoken','transformers','tokenizers','datasets','sentencepiece','huggingface_hub','matplotlib','regex']:
...
def main():
    ...
    for tok_name in TOKENIZERS:
        ...
        for lang in LANGS:
            for item in corpus:
                text=item[lang]; rows.append({'sentence_id':item['id'],'language':lang,'text':text,'tokenizer':tok_name,'token_count':len(encode(text)),'word_count':len(re.findall(r'\S+',text)),'unicode_codepoints':len(text),'grapheme_clusters':graphemes(text),'utf8_bytes':len(text.encode('utf-8'))})
    for tok in TOKENIZERS:
    ...
    write_csv(RESULTS/'sentence_level_measurements.csv',rows,['sentence_id','language','text','tokenizer','token_count','word_count','unicode_codepoints','grapheme_clusters','utf8_bytes'])
    for lang in LANGS:
        for tok in TOKENIZERS:
            group=[r for r in rows if r['language']==lang and r['tokenizer']==tok]; tv=[r['token_count'] for r in group]; tt=sum(tv); tw=sum(r['word_count'] for r in group); tc=sum(r['unicode_codepoints'] for r in group); tg=sum(r['grapheme_clusters'] for r in group); tb=sum(r['utf8_bytes'] for r in group); s=stats(tv)
            summary[(lang,tok)]={'language':lang,'tokenizer':tok,'num_sentences':len(group),'mean_tokens_per_sentence':s['mean'],'median_tokens_per_sentence':s['median'],'std_tokens_per_sentence':s['std'],'p25_tokens_per_sentence':s['p25'],'p75_tokens_per_sentence':s['p75'],'p90_tokens_per_sentence':s['p90'],'p95_tokens_per_sentence':s['p95'],'mean_tokens_per_word':statistics.fmean(r['token_count']/r['word_count'] for r in group),'aggregate_tokens_per_word':tt/tw,'mean_tokens_per_char':statistics.fmean(r['token_count']/r['unicode_codepoints'] for r in group),'aggregate_tokens_per_char':tt/tc,'mean_tokens_per_grapheme':statistics.fmean(r['token_count']/r['grapheme_clusters'] for r in group),'aggregate_tokens_per_grapheme':tt/tg,'mean_tokens_per_byte':statistics.fmean(r['token_count']/r['utf8_bytes'] for r in group),'aggregate_tokens_per_byte':tt/tb,'relative_to_english':0.0}
    for tok in TOKENIZERS:
